...
// prevAccum(N프레임 안정 평균)을 기준으로 accumulated 방향으로 조금씩 추적.
// 이번 프레임 ray spike는 accumulated에 1/(N+1) 비율로만 반영되고,
// 추가로 alpha=0.15 감쇠 → display spike 기여 ≈ 0.15/(N+1).
// g_accumulation은 raw 값 유지. race condition 없음.
